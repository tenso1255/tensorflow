...
using tsl::profiler::kNamescopeStackTreePlaneName;
...
void CuptiTraceCollector::OnTracerCollectedCallbackData(
    std::vector<CallbackAnnotationsAndEvents> annotations_and_events_list,
    bool need_callback_events) {
  // Merge per-thread scope range call stack.
  for (auto& annotations_and_events : annotations_and_events_list) {
    auto& per_thread_scope_range_call_stack =
        annotations_and_events.scope_range_call_stack();
    for (const auto [child_id, parent_id] : per_thread_scope_range_call_stack) {
      namescope_stack_tree_.insert({child_id, parent_id});
    }
    // Free resources earlier although it will be freed later if not here.
    per_thread_scope_range_call_stack.clear();
  }
...
class CuptiTraceCollectorImpl : public CuptiTraceCollector {
 public:
  ...
  void Flush() override {}
  void ExportNamescopeCallIdTree(XSpace* space) {
    XPlaneBuilder plane(
        FindOrAddMutablePlaneWithName(space, kNamescopeStackTreePlaneName));
    ...
    for (const auto& [child_id, parent_id] : namescope_stack_tree_) {
  ...
  bool Export(XSpace* space, uint64_t end_gpu_ns) override {
    ...
    ExportNamescopeCallIdTree(space);
    size_t num_events = 0;
