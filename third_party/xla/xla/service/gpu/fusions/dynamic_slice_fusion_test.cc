...
TEST_F(DynamicSliceFusionTest, ReduceScatterDUSLoopIterationOffset) {
  ...
  TF_ASSERT_OK_AND_ASSIGN(auto expected, GetOptimizedModule(hlo_ref));
  HloModuleConfig config;
  DebugOptions debugoptions = GetDebugOptionsForTest();
  debugoptions.set_xla_gpu_enable_address_computation_fusion(true);
  config.set_debug_options(debugoptions);
  ...
  std::vector<HloComputation*> address_computations_ref =
      GetAddressComputations(*expected);
  ...
  EXPECT_TRUE(RunAndCompareTwoModulesReplicated(
      std::move(expected), std::move(module_with_adddress_computation), false,
      true, error));
}
