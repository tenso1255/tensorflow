...
absl::StatusOr<llvm::SmallVector<int64_t>> FindOutputTileSizesForEpilogue(
  ...
  auto get_tile_sizes = [&](int64_t rank) {
    CHECK_GE(rank, 2);
    // We always expect the shape to be [1, ..., block_m, block_n].
  ...
  if (VLOG_IS_ON(1)) {
    ...
    LOG(INFO) << "All ops in the computation: " << computation->ToString();
    for (const auto& instr : tiled_instructions) {
...
// the given accessor.
template <typename T>
HloInstructionSet GetRecursiveInstructionSet(HloInstruction* instruction,
...
// Verifies that the set of instructions is closed, i.e. that the set of
// instructions reachable through the given accessor are either in the set
// itself or the root.
template <typename T>
...
absl::Status HoistBitcastUpwardsToCallers(
  ...
  HloInstructionSet producers =
      GetRecursiveInstructionSet(bitcast, &HloInstruction::operands);
  TF_RETURN_IF_ERROR(
      VerifyIsClosedInstructionSet(producers, bitcast, &HloInstruction::users));
...
absl::Status HoistBitcastDownwardsToCallers(
    HloInstruction* bitcast, const std::vector<HloInstruction*>& callers) {
  HloInstructionSet consumers =
      GetRecursiveInstructionSet(bitcast, &HloInstruction::users);
  TF_RETURN_IF_ERROR(VerifyIsClosedInstructionSet(consumers, bitcast,
                                                  &HloInstruction::operands));
  if (auto is_root = [](HloInstruction* instr) { return instr->IsRoot(); };
      !is_root(bitcast) && absl::c_none_of(consumers, is_root)) {
    return absl::InternalError(
        absl::StrCat("Expected", bitcast->ToString(),
...
// Try hoisting bitcasts in the computation away from 'dot' to its callers.
// Some bitcasts may remain in the computation, because they cannot be hoisted
// across all ops (e.g. across a transpose). This is not reported as an error.
absl::Status HoistBitcastsInComputationToCallers(HloInstruction* dot,
                                                 CallGraph* call_graph) {
  auto callers = call_graph->GetComputationCallers(dot->parent());
  for (HloInstruction* instruction :
       GetRecursiveInstructionSet(dot, &HloInstruction::operands)) {
    if (instruction->opcode() != HloOpcode::kBitcast) {
      continue;
    }
    VLOG(1) << "Hoisting bitcast upwards " << instruction->ToString();
    auto status = HoistBitcastUpwardsToCallers(instruction, callers);
    if (!status.ok()) {
      VLOG(1) << "Failed to hoist bitcast upwards: " << status;
    }
  }
  for (HloInstruction* instruction :
       GetRecursiveInstructionSet(dot, &HloInstruction::users)) {
    if (instruction->opcode() != HloOpcode::kBitcast) {
      continue;
    }
    VLOG(1) << "Hoisting bitcast downwards " << instruction->ToString();
    auto status = HoistBitcastDownwardsToCallers(instruction, callers);
    if (!status.ok()) {
      VLOG(1) << "Failed to hoist bitcast downwards: " << status;
    }
  }
  return absl::OkStatus();
}
...
class NestGemmFusionVisitor : public DfsHloRewriteVisitor {
 public:
  ...
  absl::Status HandleFusion(HloInstruction* instruction) override {
    ...
    TF_RETURN_IF_ERROR(HoistBitcastsInComputationToCallers(dot, call_graph_));
    VLOG(1) << "After hoisting bitcasts: " << computation->ToString();
