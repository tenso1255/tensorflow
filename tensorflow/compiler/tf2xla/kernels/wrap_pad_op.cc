  ...
  public:
    ...
    absl::StatusOr<xla::XlaOp> DoWrapPad(const xla::XlaOp t,
      ...
      for (int64_t dimno = original_shape.rank() - 1; dimno >= 0; --dimno) {
        ...
        auto lhs_pad = xla::SliceInDim(accum, dim_size - lhs_padding, dim_size,
                                       1, dimno);
...
class WrapPadGradOp : public XlaOpKernel {
  public:
    ...
    absl::StatusOr<xla::XlaOp> DoWrapPadGrad(
        const xla::XlaOp t, const xla::Shape& original_shape,
        const xla::LiteralSlice& pad_literal, xla::XlaBuilder* b) {
      xla::XlaOp grad = t;
      for (int64_t dimno = original_shape.rank() - 1; dimno >= 0; --dimno) {
        int64_t lhs_padding = pad_literal.Get<int64_t>({dimno, 0});
        int64_t rhs_padding = pad_literal.Get<int64_t>({dimno, 1});
        ...
        xla::XlaOp padded_lhs_pad = xla::PadInDim(
            lhs_pad, xla::ScalarLike(lhs_pad, 0), dimno,
        ...
        xla::XlaOp padded_rhs_pad = xla::PadInDim(
            rhs_pad, xla::ScalarLike(rhs_pad, 0), dimno,
            /*pad_lo=*/0,
